          ...
          <>
            ...
            {user?.user_type === 'admin' && is신규단가PanelOpen && (
              <div className="admin-shingyu-panel" role="region" aria-label="신규 단가 안내">
                ...
                <div className="admin-shingyu-panel-actions">
                  {typeof onAdmin신규단가 === 'function' && (
                    <button type="button" className="toolbar-order-btn" onClick={() => onAdmin신규단가()}>
                      신규단가 등록 열기
                    </button>
                  )}
                ...
                {(Array.isArray(filteredProducts) ? filteredProducts : []).map((p, rowIndex) => {
                  ...
                  <div key={p._id || p.name} className={`product-list-row group-color-${colorIndex}`}>
                    <span className="col-name">
                      ...
                      <span className="product-route-meta product-route-meta--right" title="품명·규격·DB 대분류 기준 자동 분류">
